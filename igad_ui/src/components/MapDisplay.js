...
const MapComponent = forwardRef(({ layersVisibility, filteredLayers, onToggleLayer, onMapInitialized }, ref) => {
  ...
  const updateTreeControl = useCallback(() => {
    ...
    const pestsTree = {
      ...
      children: [
        ...
        layersVisibility["Hoppers"] && {
          label: `Hoppers <div style="background-color: green; width: 15px; height: 15px; border-radius: 50%; display: inline-block;"></div>`,
          layer: overlayLayersRef.current["Hoppers"],
        },
      ].filter(Boolean), // Filter out any undefined items (for layers that are off)
      collapsed: false,
    };
    ...
    const climateTree = {
      ...
      children: Object.entries(overlayLayersRef.current)
        .filter(([layerName]) => layersVisibility[layerName]) // Only show if visible
  ...
  const updateFilteredLayers = useCallback(() => {
    ...
    Object.entries(filteredLayers).forEach(([layerName, filterData]) => {
      ...
      if (layer) {
        if (filterData && filterData.cqlFilter) {
          // Apply filter
          const filteredLayer = L.tileLayer.wms(layer._url, {
            ...layer.options,
            cql_filter: filterData.cqlFilter,
            random: Math.random() // Force refresh
          });
          
          mapRef.current.removeLayer(layer);
          mapRef.current.addLayer(filteredLayer);
          ...
          // Clear filter
          const originalLayer = L.tileLayer.wms(layer._url, {
            ...layer.options,
            cql_filter: null,
            random: Math.random() // Force refresh
          });
          ...
          mapRef.current.removeLayer(layer);
          mapRef.current.addLayer(originalLayer);
    ...
    updateTreeControl();
  }, [filteredLayers, layersVisibility, onToggleLayer, updateTreeControl]);


  const handleMapClick = useCallback(async (e) => {
    const latlng = e.latlng;
    const boundaryLayer = overlayLayersRef.current['Boundary'];
    const riskLayer = overlayLayersRef.current['Desert Locust Risk'];
    ...
    try {
      ...
      if (boundaryData.features && boundaryData.features.length > 0) {
        ...
        const bbox = turf.bbox(boundaryGeometry);
        ...
        if (riskLayer) {
          ...
          if (riskUrl) {
            ...
            if (riskData.features && riskData.features.length > 0) {
              const riskLevel = riskData.features[0].properties.risk || 'Unknown';
              popupContent += `<strong>Risk Level:</strong> ${riskLevel}<br>`;
            }
          }
        }
        ...
        for (const layerName of pestLayers) {
          const layer = overlayLayersRef.current[layerName];
          const layerUrl = `${layer._url}?service=WMS&version=1.1.0&request=GetFeatureInfo&layers=${layer.options.layers}&query_layers=${layer.options.layers}&info_format=application/json&srs=EPSG:4326&bbox=${bbox.join(',')}&width=256&height=256&x=128&y=128&feature_count=1000`;
          ...
          if (layerData.features && layerData.features.length > 0) {
            ...
            const pestPoints = layerData.features.filter((feature) => 
              turf.booleanPointInPolygon(turf.point(feature.geometry.coordinates), boundaryGeometry)
            );
  
            if (pestPoints.length > 0) {
              const latestDate = new Date(Math.max(...pestPoints.map(point => new Date(point.properties.finishdate))));
              pestData[layerName] = latestDate;
            }
          }
        }
  
        if (Object.keys(pestData).length > 0) {
          popupContent += '<strong>Latest Infestation Dates:</strong><br>';
          for (const [layerName, date] of Object.entries(pestData)) {
            popupContent += `${layerName}: ${date.toISOString().split('T')[0]}<br>`;
          }
        } else {
          popupContent += '<strong>Pest Infestation:</strong> No data available<br>';
        }
